...
- ✅ **Panel de administración** (`frontend/` bajo `/panel`): login JWT y, en el menú, **Agenda** (día, alta manual de cita servicio→día→hueco→cliente, y cambiar estado/cancelar), **Clientes** (búsqueda, paginación y ficha con historial y fidelización), **Servicios** (catálogo + sedes donde se ofrece), **Personal** (profesionales con sedes/servicios + horario semanal por sede), **Sedes** (alta/edición de salones), **Bloqueos** (vacaciones/ausencias del personal), **WhatsApp** (bandeja de atención humana: leer/responder/resolver), **Lista de espera** (ver/dar de baja), **Valoraciones** (reseñas), **Informes** (ingresos, canal, no-show, retención, valoraciones, ocupación y horas punta), **Cuenta** (plan/límites/facturación Stripe) y **Apariencia** (white-label). Cerrar sesión revoca el token.
