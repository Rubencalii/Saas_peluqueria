...
- 🟡 **Panel de administración** (`frontend/` bajo `/panel`): login JWT, **agenda** del día (cambiar estado/cancelar citas), **clientes** (búsqueda, paginación y ficha con historial y fidelización), **cuenta** (plan, límites y facturación con Stripe) y **apariencia** (white-label: nombre, colores y logo con vista previa). Núcleo funcional; faltan pantallas de configuración (servicios, personal, horarios, sedes, informes, lista de espera, conversaciones).
- ✅ **White-label por empresa** (doc 08): cada cuenta personaliza nombre, colores y logo desde *Apariencia*; el tema se aplica a su **web de reserva** (por subdominio) y a su **panel**. Las páginas públicas se renderizan por host (sin caché compartida entre tenants).
