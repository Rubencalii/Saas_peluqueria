...
- 🟡 **Panel de administración** (`frontend/` bajo `/panel`): login JWT, **agenda** del día (cambiar estado/cancelar citas), **clientes** (búsqueda, paginación y ficha con historial y fidelización), **servicios** (alta/edición del catálogo y sedes donde se ofrece), **cuenta** (plan, límites y facturación con Stripe) y **apariencia** (white-label: nombre, colores y logo con vista previa). Faltan pantallas de configuración (personal, horarios, sedes, informes, lista de espera, conversaciones) y alta manual de cita.
